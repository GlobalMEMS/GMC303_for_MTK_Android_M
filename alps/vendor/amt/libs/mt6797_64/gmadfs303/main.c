...
)
{
	...
	ALOGI_IF(adb_Debug,"%s:%d,gmadfs Cali aaa=%d\n",__func__,__LINE__,mag->status);
...
static void* thread_main(void* args)
{
	...
	static GMSENSOR_DATA sv_mag;
	GMSENSOR_DATA sv_ori;
	GMFLOAT tmpx, tmpy, tmpz;
	int16 tmp_accuracy;
	int mag_defreq=0;

	ALOGI_IF(adb_Debug,"%s:%d thread_main...\n",__func__,__LINE__);
	prms = (GMEPRMS *)args;
	minimum = -1;
	

	/* Initialize library functions and device */
	ALOGI_IF(adb_Debug,"%s:%d,GMFS_Start\n",__func__,__LINE__);
	...
	while (g_stopRequest != GME_TRUE) {
		...
		if ((flag & MAG_DATA_READY) || (flag & FUSION_DATA_READY)) {
			...
			ALOGI_IF(adb_Debug,"%s:%d GME_D_SetMode & GME_D_GetMagneticData\n",__func__,__LINE__);
			...
			{
			// low pass filter
				sv_mag.x = sv_mag.x*0.75+0.25*prms->fv_hvec.u.x;
				sv_mag.y = sv_mag.y*0.75+0.25*prms->fv_hvec.u.y;
				sv_mag.z = sv_mag.z*0.75+0.25*prms->fv_hvec.u.z;

				if(	sv_mag.status ==0 && tmp_accuracy==3){
					ALOGI_IF(adb_Debug,"status=%d,tmp=%d",sv_mag.status, tmp_accuracy);
					ALOGI_IF(adb_Debug,"GMFS_SaveParameters.\n");
			...
			ALOGI_IF(adb_Debug,"%s:%d,minimum=%lld,mag_defreq=%d\n",__func__,__LINE__,minimum,mag_defreq);
			mag_defreq++;
			if(minimum > 1000)
				minimum /= 1000000;
			ALOGI_IF(adb_Debug,"%s:%d,minimum=%lld,mag_defreq=%d\n",__func__,__LINE__,minimum,mag_defreq);
		...
		ALOGI_IF(adb_Debug,"%s:%d,ACC=(%8.3f,%6.3f,%6.3f),MAG=(%8.3f,%8.3f,%8.3f),Ori=(%8.3f,%8.3f,%8.3f)\n",__func__,__LINE__,\
			sv_acc.x,sv_acc.y,sv_acc.z,\
			sv_mag.x,sv_mag.y,sv_mag.z,\
			sv_ori.x,sv_ori.y,sv_ori.z);
		ALOGI_IF(adb_Debug,"%s:%d,fv_hvec=(%8.3f,%8.3f,%8.3f)\n",__func__,__LINE__,\
...
static int startClone(void *mem)
{
	...
	g_stopRequest = 0;
	ALOGI_IF(adb_Debug,"%s:%d startClone...\n",__func__,__LINE__);
...
int main(int argc, char **argv)
{
	...
	Disp_StartMessage();
	ALOGI_IF(adb_Debug,"%s,%d,GlobalMems Daemon start...\n",__func__,__LINE__);
#if ENABLE_GMEDEBUG
	...
	ALOGI_IF(adb_Debug,"%s:%d, g sensor dev=%d\n",__func__,__LINE__,dev);
	
	ALOGI_IF(adb_Debug,"%s,%d,adb_Debug=%d",__func__,__LINE__,adb_Debug);
	...
	{
		ALOGI_IF(adb_Debug,"%s:%d GMFS_SelfTest PASS\n",__func__,__LINE__);
	}
	//}
	...
	ALOGI_IF(adb_Debug,"%s:%d g_mainQuit= %d\n",__func__,__LINE__,g_mainQuit);
	...
	while (g_mainQuit == GME_D_FALSE) {
		...
		if (GME_D_GetOpenStatus(&st) != GME_D_SUCCESS) {
			ALOGI_IF(adb_Debug,"%s:%d GME_D_GetOpenStatus...GME_D_FAIL.\n",__func__,__LINE__);
			retValue = ERROR_GETOPEN_STAT;
			goto MAIN_QUIT;
		}
		else			
			ALOGI_IF(adb_Debug,"%s:%d GME_D_GetOpenStatus...GME_D_SUCCESS.\n",__func__,__LINE__);
		
		if (st == 0) {			
			ALOGI_IF(adb_Debug,"%s:%d Suspended.\n",__func__,__LINE__);
			GMEDEBUG(GMEDATA_LOOP, "Suspended.");
		} else {
			ALOGI_IF(adb_Debug,"%s:%d st= %d\n",__func__,__LINE__,st);
			GMEDEBUG(GMEDATA_LOOP, "Compass Opened.");
			/* Reset flag */
			g_stopRequest = 0;
			/* Start measurement thread. */
			GME_D_Reset();
			ALOGI_IF(adb_Debug,"%s:%d Reset...\n",__func__,__LINE__);
			ALOGI_IF(adb_Debug,"%s:%d startClone...\n",__func__,__LINE__);
			...
			ALOGI_IF(adb_Debug,"%s:%d GetCloseStatus...\n",__func__,__LINE__);
			if (GME_D_GetCloseStatus(&st) != GME_D_SUCCESS) {
				ALOGI_IF(adb_Debug,"%s:%d GetCloseStatus...GME_D_FAIL.\n",__func__,__LINE__);
				retValue = ERROR_GETCLOSE_STAT;
				g_mainQuit = GME_D_TRUE;
			}
			else			
			ALOGI_IF(adb_Debug,"%s:%d GetCloseStatus...GME_D_SUCCESS.\n",__func__,__LINE__);
			/* Wait thread completion. */
			g_stopRequest = 1;
			ALOGI_IF(adb_Debug,"%s:%d pthread_join. g_stopRequest = %d\n",__func__,__LINE__,g_stopRequest);
...
MAIN_QUIT:
	ALOGI_IF(adb_Debug,"%s,%d,GlobalMems Daemon quit...\n",__func__,__LINE__);
	// Release library 
